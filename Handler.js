...
function downHandler(event)		//jemand hat eine taste gedrÃ¼ckt
{
	switch(event.keyCode)		//welche taste?
	{
		...
		case 17://Strg runter
			...
			Strg_down = true;
			if( mario.shooter )
				mario.shoot();	//Mario schieÃŸen lassen
			break;
		case 89://y runter
			mario.fast = true;	//Mario schnell laufen lassen
			y_down = true;
		...
		case 40://Pfeiltaste nach unten runter
			...
			mario.walking = false;				//wÃ¤hrend dem Ducken kann man Mario nicht seitlich bewegen!!!
			mario.stand();						//heiÃŸt zwar stand(), macht aber crouch()   ;-)
			break;
		case 39://Rechte Pfeiltaste runter
			rightkey_down = true;
			if( !mario.crouching )				//beim Ducken sollen RichtungsÃ¤nderungen ignoriert werden
			{
				mario.direction = 'right';
				mario.walking = true;			//fÃ¼r horizontale Bewegung in move()-Methode
				if( !mario.moving )
					{
						mario.moving = true;	//lÃ¶se move() aus, sofern noch nicht geschehen
						mario.move();
					}
			}
			break;
...
function upHandler(event)		//jemand hat eine gedrÃ¼ckte taste losgelassen
{
	switch(event.keyCode)		//welche taste?
	{
		case 17://Strg hoch
			Strg_down = false;
			if( !y_down )
				mario.fast = false;	//Mario lÃ¤uft wieder mit normaler Geschwindigkeit
			break;
		case 89://y hoch
			y_down = false;
			if( !Strg_down )
		...
		case 40://Pfeiltaste nach unten hoch
			...
			if( rightkey_down || leftkey_down )		//wÃ¤hrend des duckens war eine blockierte richtungstaste gedrÃ¼ckt
			{
		...
		case 39://Rechte Pfeiltaste hoch
			...
			if( !mario.crouching )				//beim Ducken sollen RichtungsÃ¤nderungen ignoriert werden
			{
				if(leftkey_down==true)	//es waren beide Richtungstasten gedrÃ¼ckt
					mario.direction = 'left';
				else					//keine Richtungstaste mehr gedrÃ¼ckt
		...
		case 37://Linke Pfeiltaste hoch
			...
			if( !mario.crouching )				//beim Ducken sollen RichtungsÃ¤nderungen ignoriert werden
			{
				if(rightkey_down==true)	//es waren beide Richtungstasten gedrÃ¼ckt
					mario.direction = 'right';
				else					//keine Richtungstaste mehr gedrÃ¼ckt
					mario.walking = false;	//stoppe horizontale Bewegung in move()-Methode
			}
...
function newgameHandler()
{
	...
	else					//keine Cookies -> starte neues Spiel
	{
		...
		custom = false;
		LoadLevel();		//Zeichne Level, erzeuge Spielfigur, lade Gegner und Items gemÃ¤ÃŸ levelarray....
							//startet auÃŸerdem die GameLoop()-Schleife, sobald alle animierten Objekte (Mario, Gegner, MÃ¼nzen, etc.) vollstÃ¤ndig geladen sind
		$('#menu').hide();
		$('#background').show();
		menuMusic.pause();
	}
...
{	
	document.onkeydown = NodownHandler;		//lÃ¶sche Tastenbelegung fÃ¼r Mario-Steuerung
	...
	setTimeout( function() 
						{
							//sÃ¤mtliche MÃ¼nzen aus Ã¤lteren SpieldurchgÃ¤ngen lÃ¶schen, z.B. nachdem man gestorben ist und das Level neu geladen wurde: 
							for( var k = coins.length ; k-- ; )
								coins[k].body.destroy().remove();			//spritely beenden und MÃ¼nzen-Div vernichten
							coins = new Array();	
							
							//alle alten MÃ¼nzboxen lÃ¶schen:
							for( var k = coinboxes.length ; k-- ; )
							{
								coinboxes[k].body.remove();					//Block-Div entfernen
								coinboxes[k].coin.remove();					//MÃ¼nzen-Div vernichten, falls es noch existiert
							}
							coinboxes = new Array();	
							
							//alle alten Vielfach-MÃ¼nzboxen lÃ¶schen:
							for( var k = multiple_coinboxes.length ; k-- ; )
							{
								multiple_coinboxes[k].body.remove();		//Block-Div entfernen
								multiple_coinboxes[k].coin.remove();		//MÃ¼nzen-Div vernichten, falls es noch existiert
							}
							multiple_coinboxes = new Array();	
							
							//lÃ¶sche alle alten Sternboxen:
							...
							for( var k = plants.length ; k-- ; )
							{
								...
								if( plants[k].frame )
									plants[k].frame.remove();				//den Rahmen bei den RÃ¶hrenpflanzen entfernen
							}
							plants = new Array();
							
							//LÃ¶sche alle Gegner aus Ã¤lteren SpieldurchgÃ¤ngen:
							for( var k = enemies.length ; k-- ; )
							{
								enemies[k].body.destroy().remove();			//spritely beenden und Gegner-Div vernichten
								if( enemies[k].shell )
									enemies[k].shell.destroy().remove();	//SchildkrÃ¶tenpanzer vernichten
							}
							enemies = new Array();
