...
function readFile() {
    ...
    if(!files.length) { // keine Datei gewaehlt
      $('<p></p>').attr('class','load_warning').css({'font-size': '12px'}).appendTo('#form_load_file').html('¡Por favor seleccione un archivo!');
      return;
    }
	var file = files[0];
	...
	reader.onload = function(e) {
		...
		{
		  $('<p></p>').attr('class','load_warning').css({'font-size': '12px'}).appendTo('#form_load_file').html('¡No se seleccionó ningún archivo compatible!');
		  return;
		}
